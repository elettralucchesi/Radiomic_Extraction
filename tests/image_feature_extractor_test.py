...
def test_radiomic_extractor_3D_valid_input_feature1():
    ...
    img_1 = sitk.GetImageFromArray(np.random.rand(3, 3, 3))
    mask_1 = sitk.GetImageFromArray(np.full((3, 3, 3), fill_value=1, dtype=np.uint16))  # Assuming this will result in a label of 1
...
@pytest.mark.parametrize("patient_dict_2D, extractor, expected_message", [
    (123, featureextractor.RadiomicsFeatureExtractor(), "patient_dict_2D must be a dictionary."),
    ({}, "invalid_extractor", "extractor must be an instance of RadiomicsFeatureExtractor."),
])
def test_radiomic_extractor_2D_type_errors(patient_dict_2D, extractor, expected_message):
    """
...
def test_radiomic_extractor_2D_no_labels_in_mask():
    ...
    patient_dict_2D = {
        123: [{"ImageVolume": img_1, "MaskVolume": mask_1, "Label": 0, "SliceIndex": 0}],
    }
...
def test_radiomic_extractor_2D_valid_input_patient_key():
    ...
    mask_1 = sitk.GetImageFromArray(np.full((3, 3), fill_value=1, dtype=np.uint16))  # Label 1
    ...
    assert "123-0-1" in result, f"Expected patient key '123-0-1' not found in the result."
...
def test_radiomic_extractor_2D_valid_input_feature1():
    ...
    assert result["123-0-1"]["Feature1"] == 0.5, f"Expected Feature1 value of 0.5, but got {result['123-0-1']['Feature1']}"
...
def test_radiomic_extractor_2D_valid_input_feature2():
    ...
    result = radiomic_extractor_2D(patient_dict_2D, extractor)
    assert result["123-0-1"]["Feature2"] == 0.8, f"Expected Feature2 value of 0.8, but got {result['123-0-1']['Feature2']}"
...
@pytest.mark.parametrize("patient_dict, extractor, expected_message", [
    (["invalid_patient_data"], featureextractor.RadiomicsFeatureExtractor(), "patient_dict must be a dictionary."),
    ({}, "invalid_extractor", "extractor must be an instance of RadiomicsFeatureExtractor."),
])
def test_extract_radiomic_features_type_errors(patient_dict, extractor, expected_message):
    """
